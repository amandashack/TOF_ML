...
file_list = [file for file in os.listdir(folder_path) if file.endswith('grouped.csv')]  #this will differ
...
vmin = np.percentile(tof_data, 0)  # 0th percentile of the Time of Flight data
vmax = np.percentile(tof_data, 99.9)  # xth percentile 
...
if not hit_sensor_data.empty and not x_data.empty:
    ...
    sm.set_array([])
    ticks = np.linspace(norm.vmin, norm.vmax, 8)  # Adjust the number of ticks to represent 80% of the colors
    cbar = plt.colorbar(sm, ax=ax, ticks=ticks, pad=0.2)
    ...
    output_path = r'C:\Users\lauren\Documents\Simion_Simulation\simulation_files\histo_plot.pdf' #change name for output pdf
